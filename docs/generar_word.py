...
def heading(text, level=1):
    ...
    if level == 1:
        ...
        p.runs[0].font.color.rgb = RGBColor(0x20, 0x60, 0x99)
    return p
...
def code_block(text):
    """Monospaced grey block for code/diagrams."""
    p = doc.add_paragraph()
    ...
    for line in text.split("\n"):
        if p.runs:          # subsequent lines → new paragraph same style
            p = doc.add_paragraph()
            p.paragraph_format.left_indent  = Cm(0.5)
            p.paragraph_format.right_indent = Cm(0.5)
            pPr = p._p.get_or_add_pPr()
...
r = t.add_run("Informe de Arquitectura — ViveBien (TrackLife)")
...
for line in [
    ...
    rb = p.add_run(line[0] + " ")
    rb.bold = True; rb.font.size = Pt(11); rb.font.name = "Calibri"
    rv = p.add_run(line[1])
...
# 1. PARTES Y MÓDULOS
# ══════════════════════════════════════════════════════════════════════════════
heading("1. Partes y Módulos de la Aplicación", 1)

heading("Módulos lógicos (Monolito Modular)", 2)

code_block("""app/src/main/java/com/jpalomino502/vivebien/
│
...
doc.add_paragraph()
heading("Tecnologías y librerías utilizadas", 2)
...
# ══════════════════════════════════════════════════════════════════════════════
heading("2. Repositorio — Evidencia del Estado Inicial", 1)

para("URL fork: https://github.com/IloveyoubobaUNAB/Tracklife")
para("Branch: main")
para("Commit inicial (app base): 345c736 — \"Create app vivebien\" (21/03/2025)")

doc.add_paragraph()
heading("Estructura original (arquitectura inicial — Big Ball of Mud)", 2)
...
doc.add_paragraph()
heading("Problemas identificados en la arquitectura inicial", 3)
problems = [
...
# ══════════════════════════════════════════════════════════════════════════════
heading("3. Historial de Commits — Evolución de la Arquitectura", 1)

commits = [
    {
        "id": "555cdfc",
        "title": 'feat: agregar Hilt, Room y DataStore al sistema de build',
        "date": "11/05/2026",
        "changes": "6 archivos cambiados, 43 inserciones",
        "files": "gradle/libs.versions.toml, build.gradle.kts, app/build.gradle.kts",
        "reason": ("La app no tenía DI funcional ni persistencia. Se agregaron las dependencias base "
                   "requeridas por la nueva arquitectura: Hilt 2.51.1 con KSP (compatible con Kotlin 2.0.0, "
                   "kapt está obsoleto), Room 2.6.1 y DataStore 1.1.1. Sin estas dependencias ningún paso "
                   "posterior era posible."),
    },
    {
        "id": "6fd140a",
        "title": "feat: agregar modelos de dominio y módulo de inyección de dependencias (Hilt)",
        "date": "11/05/2026",
        "changes": "6 archivos cambiados, 85 inserciones",
        "files": "ViveBienApp.kt, AndroidManifest.xml, core/domain/model/*.kt, core/di/AppModule.kt",
        "reason": ("Clean Architecture requiere modelos de dominio puros (sin dependencias de Android). "
                   "Se crearon User, ActivityRecord, Appointment, Medication, VitalSign. Se registró "
                   "ViveBienApp con @HiltAndroidApp para activar el grafo de dependencias. AppModule define "
                   "los bindings de repositorios."),
    },
    {
        "id": "f47d2b5",
        "title": "feat: implementar capa de autenticación con MVVM y Clean Architecture",
        "date": "11/05/2026",
        "changes": "6 archivos cambiados, 160 inserciones, 28 eliminaciones",
        "files": "AuthRepository, AuthRepositoryImpl, LoginUseCase, LoginViewModel, LoginScreen (refactorizado)",
        "reason": ("La pantalla de login tenía lógica de negocio directamente en el Composable (acceso a "
                   "Constants.TEST_USER). Se extrajo a capas separadas: el UseCase valida las reglas, el "
                   "Repository abstrae el almacenamiento (DataStore), el ViewModel gestiona el estado con "
                   "StateFlow. La pantalla quedó como observador puro del estado. Se eliminaron las "
                   "credenciales hardcodeadas de Constants.kt."),
    },
    {
        "id": "b9a65bd",
        "title": "feat: crear ViewModels con StateFlow para cada feature de la aplicación",
        "date": "11/05/2026",
        "changes": "5 archivos cambiados, 247 inserciones",
        "files": "HomeViewModel, HealthViewModel, ActivityViewModel, AppointmentsViewModel, ProfileViewModel",
        "reason": ("Las 5 pantallas principales carecían de ViewModel, gestionando su \"estado\" con datos "
                   "hardcodeados. Se crearon ViewModels con @HiltViewModel e @Inject constructor para cada "
                   "feature, exponiendo StateFlow<UiState>. Esto establece el contrato UI ↔ ViewModel que "
                   "permite reemplazar datos estáticos por datos reales en el siguiente paso."),
    },
    {
        "id": "ffc6e44",
        "title": "refactor: extraer componente CustomTabRow y separar definición de rutas",
        "date": "11/05/2026",
        "changes": "3 archivos cambiados, 65 inserciones, 12 eliminaciones",
        "files": "CustomTabRow.kt (nuevo), Screen.kt (nuevo), NavGraph.kt (limpiado)",
        "reason": ("Existían 4 implementaciones idénticas del componente TabRow copiadas en cada pantalla "
                   "(duplicación DRY). Se extrajo a CustomTabRow.kt reutilizable. Adicionalmente, NavGraph.kt "
                   "contenía una sealed class Screen inline junto a la lógica de navegación, violando "
                   "separación de responsabilidades. Se separó en Screen.kt independiente."),
    },
    {
        "id": "adb7f12",
        "title": "feat: conectar todas las pantallas a sus ViewModels mediante StateFlow",
        "date": "11/05/2026",
        "changes": "5 archivos cambiados, 329 inserciones, 1.007 eliminaciones",
        "files": "HomeScreen, SaludScreen, ActividadScreen, CitasScreen, PerfilScreen refactorizados completamente",
        "reason": ("Las pantallas aún renderizaban datos hardcodeados. Se refactorizaron para usar "
                   "viewModel: XViewModel = hiltViewModel() y collectAsStateWithLifecycle(). Se eliminaron "
                   "1.007 líneas de datos estáticos y se reemplazaron por observación reactiva del estado del "
                   "ViewModel. El calendario pasó de filas hardcodeadas a cálculo dinámico con java.time."),
    },
    {
        "id": "25dc8f3",
        "title": "feat: agregar flujo de creación de cuenta (registro de usuario)",
        "date": "11/05/2026",
        "changes": "8 archivos cambiados, 306 inserciones, 9 eliminaciones",
        "files": "RegisterScreen, RegisterViewModel, RegisterUseCase, actualización de AuthRepository, Screen.Register, NavGraph",
        "reason": ("La app solo permitía login con credenciales fijas. Se implementó el flujo completo de "
                   "registro siguiendo la misma arquitectura MVVM+Clean Architecture: RegisterUseCase con "
                   "validaciones de negocio, RegisterViewModel con estado reactivo, RegisterScreen como vista "
                   "pura, persistencia en DataStore, y enlace de navegación desde el LoginScreen."),
    },
    {
        "id": "54a501d",
        "title": "feat: hacer la app completamente funcional con datos reales (monolito modular)",
        "date": "11/05/2026",
        "changes": "58 archivos cambiados, 2.039 inserciones, 706 eliminaciones",
        "files": "Base de datos Room completa, 5 repositorios reales, 20 casos de uso, ViewModels actualizados, pantallas con diálogos interactivos",
        "reason": ("Los ViewModels aún retornaban datos hardcodeados dentro del ViewModel (simulación). Se "
                   "implementó la capa de datos completa: 4 entidades Room con mappers, 4 DAOs con queries "
                   "reactivos (Flow), AppDatabase, DatabaseModule. Cada feature obtuvo su propio repositorio "
                   "que conecta Room/DataStore con el dominio. Los ViewModels pasaron a inyectar casos de uso "
                   "reales. Las pantallas recibieron diálogos funcionales para crear/eliminar datos."),
    },
]

for c in commits:
    heading(f"Commit — {c['id']} | {c['date']}", 2)
    p = doc.add_paragraph()
    r = p.add_run(c["title"])
    r.bold = True; r.italic = True; r.font.size = Pt(11); r.font.name = "Calibri"

    p2 = doc.add_paragraph()
    rb = p2.add_run("Cambios: ")
    rb.bold = True; rb.font.size = Pt(10); rb.font.name = "Calibri"
    rv = p2.add_run(c["changes"])
    rv.font.size = Pt(10); rv.font.name = "Calibri"

    p3 = doc.add_paragraph()
    rb = p3.add_run("Archivos: ")
    rb.bold = True; rb.font.size = Pt(10); rb.font.name = "Calibri"
    rv = p3.add_run(c["files"])
    rv.font.size = Pt(10); rv.font.name = "Calibri"

    p4 = doc.add_paragraph()
    rb = p4.add_run("Motivo: ")
    rb.bold = True; rb.font.size = Pt(10); rb.font.name = "Calibri"
    rv = p4.add_run(c["reason"])
    rv.font.size = Pt(10); rv.font.name = "Calibri"
    doc.add_paragraph()
...
# 4. MODELO C4 — ANTES
# ══════════════════════════════════════════════════════════════════════════════
heading("4. Modelo C4 — ANTES de los Cambios (Arquitectura Inicial)", 1)

heading("Nivel 1 — Contexto del Sistema", 2)
code_block("""┌─────────────────────────────────────────────────────────┐
...
doc.add_paragraph()
heading("Nivel 2 — Contenedores (ANTES)", 2)
code_block("""┌─────────────────────────────────────────────────────────────┐
│                   CONTENEDORES — ANTES                       │
└─────────────────────────────────────────────────────────────┘
...
doc.add_paragraph()
heading("Nivel 3 — Componentes (ANTES)", 2)
code_block("""┌─────────────────────────────────────────────────────────────┐
...
# 5. MODELO C4 — DESPUÉS
# ══════════════════════════════════════════════════════════════════════════════
heading("5. Modelo C4 — DESPUÉS de los Cambios (Arquitectura Mejorada)", 1)

heading("Nivel 1 — Contexto del Sistema (DESPUÉS)", 2)
code_block("""┌─────────────────────────────────────────────────────────────┐
│                    CONTEXTO DEL SISTEMA                      │
└─────────────────────────────────────────────────────────────┘
...
doc.add_paragraph()
heading("Nivel 2 — Contenedores (DESPUÉS)", 2)
code_block("""┌──────────────────────────────────────────────────────────────────┐
...
doc.add_paragraph()
heading("Nivel 3 — Componentes por Feature (DESPUÉS)", 2)
code_block("""┌──────────────────────────────────────────────────────────────────┐
...
doc.add_paragraph()
heading("Nivel 4 — Código: Flujo de Datos (DESPUÉS)", 2)
code_block("""┌──────────────────────────────────────────────────────────────────┐
...
# ══════════════════════════════════════════════════════════════════════════════
heading("6. Patrones de Diseño y Principios Aplicados", 1)

heading("Patrones aplicados", 2)
add_table(
...
doc.add_paragraph()
heading("Principios SOLID aplicados", 2)
solids = [
...
doc.add_paragraph()
heading("Clean Architecture — Regla de dependencias", 2)
...
# 7. SUSTENTACIÓN
# ══════════════════════════════════════════════════════════════════════════════
heading("7. Sustentación — Mejoras Implementadas", 1)

heading("Problema central resuelto", 2)
para('La aplicación original era un "Big Ball of Mud": toda la lógica, los datos y la presentación '
     'convivían en los mismos archivos Composable. Era imposible testear, mantener o escalar.')

doc.add_paragraph()
heading("Mejoras por dimensión", 2)
...
# 8. GLOSARIO
# ══════════════════════════════════════════════════════════════════════════════
heading("8. Glosario — Significado de Términos Importantes", 1)

heading("Patrones y arquitectura", 2)
...
for term, definition in glossary_arch:
    p = doc.add_paragraph()
    r1 = p.add_run(term + ": ")
    r1.bold = True; r1.font.size = Pt(11); r1.font.name = "Calibri"
...
doc.add_paragraph()
heading("Tecnologías y herramientas", 2)
...
for term, definition in glossary_tech:
    p = doc.add_paragraph()
    r1 = p.add_run(term + ": ")
    r1.bold = True; r1.font.size = Pt(11); r1.font.name = "Calibri"
    r2 = p.add_run(definition)
...
for term, definition in glossary_solid:
    p = doc.add_paragraph()
    r1 = p.add_run(term + ": ")
    r1.bold = True; r1.font.size = Pt(11); r1.font.name = "Calibri"
    r2 = p.add_run(definition)
...
doc.add_paragraph()
heading("Términos generales de Android", 2)
...
for term, definition in glossary_android:
    p = doc.add_paragraph()
    r1 = p.add_run(term + ": ")
    r1.bold = True; r1.font.size = Pt(11); r1.font.name = "Calibri"
    r2 = p.add_run(definition)
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run("Documento generado para la entrega de arquitectura móvil — ViveBien / TrackLife")
r.italic = True; r.font.size = Pt(10); r.font.name = "Calibri"
r.font.color.rgb = RGBColor(0x70, 0x70, 0x70)

# ── Save ──────────────────────────────────────────────────────────────────────
out = "Informe_Arquitectura_ViveBien.docx"
doc.save(out)
print("saved:", out)